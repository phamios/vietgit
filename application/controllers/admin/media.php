...
class Media extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('catemedia_model');
        @session_start();
    }
    ...
    function edit_catenews($id = null) {
        if ($this->session->userdata('admin_id') == null) {
            ...
            if (isset($_REQUEST['btt_submit_insertlink'])) {
                $name = $this->input->post('mediatype', true);
                $root = $this->input->post('cateroot', true);
                $type = $this->input->post('catetype', true);
                $active = $this->input->post('active', true);
    ...
    function create_new() {
        if ($this->session->userdata('admin_id') == null) {
            ...
            $data['list_cate_image'] = $this->cateimage_model->list_all_type(1,$this->session->userdata('admin_id'));
            $data['list_cate_video'] = $this->cateimage_model->list_all_type(2,$this->session->userdata('admin_id'));
            $data['list_catemedia'] = $this->catemedia_model->list_all();
            $data['list_menu'] = $this->modules_model->list_all_active();
            if (isset($_REQUEST['btt_submit_insertlink'])) {
                $mediatype=$this->input->post('mediatype');
                $cateid=$this->input->post('media_cateid_pictures');
                $medialink=$this->input->post('media_link');
                $mediaactive=$this->input->post('status');
                //$meadiacreatedate='';
                $this->catemedia_model->insert($mediatype,$cateid,$medialink,$mediaactive,$mediaactive);
...
//                }
    ...
    public function del_media_picture($id = null) {
        if ($this->session->userdata('admin_id') == null) {
            redirect('admincp/login');
        } else {
            $this->catemedia_model->delete($id);
            redirect('admincp/pictures');
        }
    }
    ...
    function pictures() {
        if ($this->session->userdata('admin_id') == null) {
            ...
            $data['list_media']=$this->media_model->list_all();
            $data['list_menu'] = $this->modules_model->list_all_active();
            $data['list_catemedia'] = $this->catemedia_model->list_all();
            $this->load->view('admincp/dashboard', $data);
    ...
    public function uploadimage() {
        if (0 < $_FILES['file']['error']) {
            ...
            $image_id = $this->catemedia_model->insert_image($file, null);
