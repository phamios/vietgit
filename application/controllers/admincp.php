...
class admincp extends CI_Controller {
        ...
        if ($this->session->userdata('admin_id') == null) {
            ...
            if(isset($_REQUEST['btt_submit'])){
                ...
                if($result <> null | $result <> 0 ){
                    ...
                    $this->sendemail('admegavn@gmail.com',$content,$title_mail);
    ...
    function sendemail($emailrecieve, $content,$title_mail){
         $this->load->model('config_model');
    ...
    function register($error = null) {
        if ($this->session->userdata('admin_id') != null) {
            ...
            if (isset($_REQUEST['go'])) { 
                ...
                if(strlen($username) < 5 || strlen($userpass) < 1 || strlen($email) < 1){
                ...
                if ($userid == null) {
                    ...
                    $content = "<h3>Xác nhận tài khoản</h3><br/>"; 
                    $content = $content."Vui lòng ấn vào link bên dưới để xác nhận tài khoản của bạn tại VIETADMOB.COM:<br/>";
                    $content = $content.site_url('verify/'.$email.'/'.md5("vietadmob1q2w3e".$username));
                    $title_mail = "[Urgent] Verify Account at VIETADMOB.COM";
                    $this->sendemail($email,$content,$title_mail);
                    redirect('dang-nhap');
                }
            }
            if($error <> null){
    ...
    function verify($email = null,$username_md5 = null){
        $this->load->model('user_model');
        $user = $this->user_model->list_verify_user();
        $result = 0;
        foreach($user as $value){
            if($email == $value->uemail && $username_md5 == md5("vietadmob1q2w3e".$value->uname)){
                $this->user_model->active_member($value->uname);
                $result = 1;
            }  
        }
        if($result == 0){
           redirect('dang-ky/'.md5($email.$username_md5));
        } else {
            redirect('dang-nhap');
        }
